...
class MoEFFN(nn.Module):
    ...
    def forward(self, x,modality):
        # #gate由两部分组成gating_network的输出和先验（1000,0100..)
        # #gate第一部分gating_network
        gate_weights = self.gating_network(x)#,x,x
        batch_size,c,h,d=gate_weights.shape
        gate_weights = gate_weights.reshape(self.num_experts,batch_size,c,-1,self.dim)
        ...
        # print(mask_weights.shape,gate_weights.shape)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor,modality:torch.Tensor) -> torch.Tensor:
        ...
        x = shortcut + x
        # output  = self.moe(self.norm2(x))#,loss
        # x = x + output
        x = x + self.mlp(self.norm2(x),modality)
        return x#,loss

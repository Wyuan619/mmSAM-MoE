...
modality = "Ultrasound"
anatomy = "AbdomenUS_AUS_train"  # anantomy + dataset name
img_name_suffix = ".png"
gt_name_suffix = ".png"
prefix = modality + "_" + anatomy + "_"

image_path = "/home/nas2/path/wangyuan/datasets/AbdomenUS/abdominal_US/abdominal_US/AUS/images/train"  # path to the nii images
gt_path = "/home/nas2/path/wangyuan/datasets/AbdomenUS/abdominal_US/abdominal_US/AUS/annotations/train"  # path to the ground truth
npy_path = "/home/nas2/path/wangyuan/datasets/npy/" + prefix[:-1]
...
names = [
    name
    for name in names
    if os.path.exists(join(image_path, name.split(gt_name_suffix)[0] + img_name_suffix))
]
...
for name in tqdm(names):  # 训练集验证集都需要预处理
    image_name = name.split(gt_name_suffix)[0] + img_name_suffix
    gt_name = name
    gt_img =  Image.open(join(gt_path, gt_name))
    gt_data_ori = np.uint8(gt_img)
    # remove label ids
    ...
    if len(z_index) > 0:
        ...
        img_sitk = sitk.ReadImage(join(image_path, image_name))
